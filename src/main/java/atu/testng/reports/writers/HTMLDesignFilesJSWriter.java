...
public class HTMLDesignFilesJSWriter {
	...
	public static void pieChartJS1(int passedSize, int failedSize, int skippedSize,ITestResult paramITestResult) {
		try {
			String reportPath=paramITestResult.getAttribute("reportDir").toString() ;//Directory.RUNDir+Directory.SEP+ATUReportsListener.getRelativePathFromSuiteLevel(paramITestResult);
			
			PrintWriter localPrintWriter = new PrintWriter(reportPath
					+ Directory.SEP + "pieChart.js");
			localPrintWriter
			.println("$(document).ready(function() {\n    var data = [['"
					+ ReportLabels.PASS.getLabel()
