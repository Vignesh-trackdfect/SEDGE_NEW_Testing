...
public class DASHPRO_BARWITHLINE_TESTING extends Keywords{
	...
	public void barWithLine_testing(WebDriver driver,int iteration,String Flag) {
		...
		String Default_BulletType_BarWithLine=Utils.getDataFromTestData("SmokeTesting_UI", "Default_BulletType_BarWithLine");	
		String Default_StrokeWidth_BarWithLine=Utils.getDataFromTestData("SmokeTesting_UI", "Default_StrokeWidth_BarWithLine");	
		String Default_X_LabelPosition_BarWithLine=Utils.getDataFromTestData("SmokeTesting_UI", "Default_X_LabelPosition_BarWithLine");	
		String Default_GridDistance_BarWithLine=Utils.getDataFromTestData("SmokeTesting_UI", "Default_GridDistance_BarWithLine");	
		...
		String Change_Line_Style_ChartStyle=Utils.getDataFromTestDataIteration(iteration,"BarWithLine_Widget_Testing", "Change_Line_Style_ChartStyle");
		String Change_X_Lable_Position_ChartStyle=Utils.getDataFromTestDataIteration(iteration,"BarWithLine_Widget_Testing", "Change_X_Lable_Position_ChartStyle");
		String Change_Grid_Distance_ChartStyle=Utils.getDataFromTestDataIteration(iteration,"BarWithLine_Widget_Testing", "Change_Grid_Distance_ChartStyle");
		
		
		setTestCaseID(" ");
		Actions action=new Actions(driver);
		//creating one new tab here
		mouseOverToElement(driver, ADDTabPlus);
  	    mouseOverAndClick(driver, ADDTabPlus);
  	    wait(driver,"1");
  	    sendKeys(driver,newlyAddedTab,"BarWithLineTestTab1");
  	    //After typing the tab name, we need to click outside the input in order to create new tab, so that here I am clicking the DashPro element.
  	    click(driver,DashProPage);
  	    elementnotvisible1(driver, RPE_Loading);
  	    //Clicking the Add widget button.
		...
		//Validating the BarWithLine Widget present 
		...
		//Validating the Image of the Bar with Line Widget and also validating the Tooltip
		WidgetValidation(driver,BarLine_Scatter_widget,"Bar with Line/Scatter");
		
		setTestCaseID("TC_BARWITHLINE_ AXES_003");
		//Clicking the BarwithLine widget and Validating it is working fine or not
		...
		String widgetValue=getText1(driver,WidgetValue);
	 	...
	 	//Verifying the Axes present and expanded by default
    	...
    	//Verifying the Line And Scatter select options are present
    	verifyElementDisplayed(driver, LineOption);
    	verifyElementDisplayed(driver, ScatterOption);
    	
    	//Based on the Input sheet selecting the Line or Scatter
    	...
    	//Verifying X axis input present and the default Value of X axis input
		...
		//Verifying Y axis input present and the default Value of Y axis input
		...
		//Verifying the error is displayed or not when click Apply Button Without providing the X and Y input Value
    	...
    	setTestCaseID("");
    	//Clicking the X Axis input and check the dropdown is expanded or not
		...
		//Getting the Available column count and validating all the column displayed with dataType Icon
		...
		//Validating the only Categorical,Date,Numerical,Text value are displayed in the X Axis dropdown
		...
		//Validating the Categorical And Text Column are displayed with unique Values in the X Axis dropdown
		...
		setTestCaseID("");
		//Validating the Scrollbar in the X Axis Input dropdown
		ScrollBarValidation1(driver,X_Axis_dropdownResultsExp,"X-Axis");
		
		setTestCaseID("TC_BARWITHLINE_ AXES_009");
		//Validating the Search input functionality with both Valid and InValid inputs in the X Axis dropdown
		if(verifyElementDisplayed(driver, dropdownInputsearchBox)) {
			...
			//Selecting the X Axis Input Value
     ...
     // *********** AxisTitle Validation Start *************
 		...
 		if(BarWithLine_Axis_Title.equals("Yes")) {
	 	...
	 	// ******************** ChartTitle Validation Start *******************
	      ...
	      if(BarWithLine_Chart_Title.equals("Yes")) {
	  ...
	  //********** ChartStyle Validation Start *************************
        ...
        if(BarWithLine_Chart_Style.equals("Yes")) {
        	...
        	//Validating Chart Format is expanded or not 
	         ...
	         scrollUsingElement(driver,ApplyButton);
	 		 ...
	 		 elementnotvisible1(driver, RPE_Loading);
	         verifyElementDisplayed(driver,Chart_Section);
   	  		 String AppliedLineStroke_act=getAttribute1(driver, Line_Stroke_Chart,"stroke");
	         ...
	         scrollUsingElement(driver,ApplyButton);
	 		 ...
	 		 elementnotvisible1(driver, RPE_Loading);
	         verifyElementDisplayed(driver,Chart_Section);
   	  		 String AppliedLineBullet_act=getAttribute1(driver, Line_Bullet_Chart,"stroke");
   	  		 if(AppliedLineBullet_act.equalsIgnoreCase(Change_Line_Bullet_Color_Chart_Style)) {
   	  			 pass(driver,"Selected "+Change_Line_Bullet_Color_Chart_Style+" color is applied in Line Bullet in the chart");
   	  		 }else {
   	  			 fail(driver,"Selected "+Change_Line_Bullet_Color_Chart_Style+" color is not applied in Line Bullet in the chart");
   	  		 }
   	         ...
   	         wait(driver,"1");
   	         click3(driver,Line_Stroke_text);
   	         String AfterRever_LineBullet=getTextBackgroundColor(driver, Line_bullet_Input_Pointer);
   	         if((AfterRever_LineBullet.equalsIgnoreCase(Default_Line_Bullet_Exp))) {
   	 			 pass(driver,"After revert all color selection default Color is updated in the Line Bullet Input");
   	         }else {
   	 			 fail(driver,"After revert all color selection default Color is not updated in the Line Bullet Input");
   	         }
   	         setTestCaseID("");
   	         click3(driver,Line_Stroke_text);
   	         click(driver,Line_bullet_Input);
   	         ...
   	         wait(driver,"1");
   	         action.keyDown(Keys.CONTROL).sendKeys("c").keyUp(Keys.CONTROL).perform();
   	         doubleClick(driver,Line_bullet_Input);
   	         wait(driver,"1");
   	         action.keyDown(Keys.CONTROL).sendKeys("v").keyUp(Keys.CONTROL).perform();
   	         wait(driver,"1");
   	         click3(driver,Line_Stroke_text);
   	         AfterPasteBackgroundColor=getTextBackgroundColor(driver, Line_bullet_Input_Pointer);
   	         if(AfterPasteBackgroundColor.equalsIgnoreCase(mouseOverHistoryColor)) {
	          	pass(driver,"Copy Paste Color value is working fine in Line Bullet Input");
   	         }else {
	          	fail(driver,"Copy Paste Color value is not working fine in Line Bullet Input");
   	         }
  			 
  			 
  			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_011");
  			 //Getting the default selected Value from the Bullet type Input and Verify it is equal to the expected Default Value
  			 String default_BulletTypeSelected=defaultSelectedValue(driver, BulletType_Input);
  			 if(default_BulletTypeSelected.equals(Default_BulletType_BarWithLine)) {
  				 pass(driver,"By default, "+Default_BulletType_BarWithLine+" is displayed in the Bullet Type Input");
  			 }else {
  				fail(driver,"By default, "+Default_BulletType_BarWithLine+" is not displayed in the Bullet Type Input");
  			 }
  			 
  			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_012");
  			 //Verifying Bullet Input Options present.
  			 click(driver,BulletType_Input);
  			 verifyElementIsPresent1(driver,Bullet_Type_Options);
  			 
  			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_013");
  			 //Selecting one Bullet option based on the Input from the Excel sheet and Validate Which is applied in the Chart
  			 selectByText(driver,BulletType_Input,Change_Bullet_Type_Chart_Style);
	         ...
	         verifyElementDisplayed(driver,Chart_Section);
	 		 ...
	 		 //Here I taking seperate Xpath for Rectangle, Circle and Triangle type in the Chart.. Based on the Selection 
	 		 if(Change_Bullet_Type_Chart_Style.equals("Rectangle")) {
	 			 if(isDisplayed2(driver,Rectangle_bullet_Type)) {
	 				 pass(driver,"Selected Rectangle bullet Option is applied in the chart");
	 			 }else {
	 				fail(driver,"Selected Rectangle bullet Option is not applied in the chart"); 
	 			 }
	 		 }else if (Change_Bullet_Type_Chart_Style.equals("Triangle")) {
	 			 if(isDisplayed2(driver,Triangle_Bullet_Type)) {
	 				 pass(driver,"Selected Triangle bullet Option is applied in the chart");
	 			 }else {
	 				fail(driver,"Selected Triangle bullet Option is not applied in the chart"); 
	 			 }
	 		 }else if(Change_Bullet_Type_Chart_Style.equals("Circle")) {
	 			 if(isDisplayed2(driver,Circle_bullet_Type)) {
	 				 pass(driver,"Selected Circle bullet Option is applied in the chart");
	 			 }else {
	 				fail(driver,"Selected Circle bullet Option is not applied in the chart"); 
	 			 }
	 		 }
	 		 
	 		 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_014");
  			 //Getting the default selected Value from the Stroke Width Input and Verify it is equal to the expected Default Value
  			 String default_StrokeWidthSelected=defaultSelectedValue(driver, Stroke_Width_Input);
  			 if(default_StrokeWidthSelected.equals(Default_StrokeWidth_BarWithLine)) {
  				 pass(driver,"By default, "+Default_StrokeWidth_BarWithLine+" is displayed in the Stroke Width Input");
  			 }else {
  				fail(driver,"By default, "+Default_StrokeWidth_BarWithLine+" is not displayed in the Stroke Width Input");
  			 }
  			 
  			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_015");
  			 //Validating the Stroke Width Options present
  			 click(driver,Stroke_Width_Input);
  			 verifyElementIsPresent1(driver, Stroke_Width_Options);
  			 
  			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_016");
  			 //Selecting the Stroke Wdith option based on the Input from the Excel sheet and Validate Which is applied in the Chart
  			 selectByText(driver,Stroke_Width_Input,Change_Stroke_Width_Chart_Style);
	         ...
	         //Getting the Stroke width value in the Chart by using Get Attribute option
  			 String AppliedStrokeWidth=getAttribute1(driver, Line_Stroke_Chart, "stroke-width");
  			 if(AppliedStrokeWidth.equals(Change_Stroke_Width_Chart_Style)) {
  				 pass(driver,"Selected Stroke Width '"+Change_Stroke_Width_Chart_Style+"' is Applied in the Chart");
  			 }else {
  				 fail(driver,"Selected Stroke Width '"+Change_Stroke_Width_Chart_Style+"' is not Applied in the Chart");
  			 }
  			 
  			
  			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_017");
  			 //Getting the default selected Value from the Line Style Input and Verify it is equal to the expected Default Value
			 String DefaultLineSelected=defaultSelectedValue(driver, Line_Style_Input);
			 ...
			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_018");
			 //validating the Line Style Options present
			 click(driver,Line_Style_Input);
			 verifyElementIsPresent1(driver, Line_Style_Options);
			 
			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_019");
			 //Changing the Line Style Value and validating the Changed Line Style Option Updated in the Chart..
			 ...
			 if(ExpectedLine.equals(chartLine)) {
  			...
  			//Testing 
 			 X_Selected=getText1(driver,X_Axis_Input);
 	         Y_Selected=getText1(driver,Y_Axis_Input);
 	        
 	         setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_020");
 	         //Validating, By default the AutoScroll toggle is enabled or not
  			 if(isToggleEnable(driver,AutoScroll_toggle)) {
  				 pass(driver,"By default, 'AutoScroll' toggle is enabled");
  				 // verifying the  X scroll and Y Scroll toggle is disabled when the Auto Scroll toggle is enabled
  				 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_021");
  				 if(isToggleEnable(driver,X_Scroll_toggle)) {
  					 fail(driver,"X Scroll toggle enabled when AutoScroll toggle is enabled for the X Value : "+Select_X_Axis_Value);
  				 }else {
  			 ...
  			 setTestCaseID("");
  			 // verifying the  X scroll and Y Scroll toggle is Accessible or not when the Auto Scroll toggle is enabled
  			 ...
  			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_022");
  			 //Clicking the AutoScroll toggle and Validate the toggle get disabled or not
  		     ...
  		     // verifying the  X scroll and Y Scroll toggle is Accessible or not when after disabling the Auto scroll toggle
  			 if(isToggleAccessible(driver,X_Scroll_toggle) && isToggleAccessible(driver,Y_Scroll_toggle)){
  			 ...
  			 setTestCaseID("");
  			 //Checking the X scroll enabling or not when click on it. Before that if already it is enabled, we are not clicking it, if already it is not enabled then click on it and check it is enabled or not
  			...
  			//Checking the Y scroll enabling or not when click on it. Before that if already it is enabled, we are not clicking it, if already it is not enabled then click on it and check it is enabled or not
  			 ...
  			 //Clicking the Auto Scroll toggle again and Validating the X Scroll and Y Scroll gets disabled
  			 click(driver,Auto_ScrollInputSlider);
  			 wait(driver,"1");
  			 ...
  			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_024");
  			 //Validating By default the EnableGrid Toggle is Enabled
  			 if(isToggleEnable(driver,EnableGrid_toggle)) {
  				 ...
  				 scrollUsingElement(driver, ApplyButton);
  	  			 ...
  	  			 elementnotvisible1(driver, RPE_Loading);
  	  			 verifyElementDisplayed(driver,Chart_Section);
  	  			 //Validating the Grid is displayed in the Chart When the toggle is enabled
  	  			 if(isDisplayed(driver,Enabled_Grid_Chart)) {
  	  			 ...
  	  			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_025");
  	  			 //Clicking the Enabled Grid toggle and Validate it is disabled
  				 ...
  				 scrollUsingElement(driver, ApplyButton);
  	  			 ...
  	  			 elementnotvisible1(driver, RPE_Loading);
  	  			 verifyElementDisplayed(driver,Chart_Section);
  	  			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_026");
  	  			 //After Disabling the Enable Grid toggle Validating it is disabled in the Chart
  	  			 if(isDisplayed(driver,Enabled_Grid_Chart)) {
  			 ...
  			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_027");
  			 //Validating the default X Label Position Input Value
			 String default_XLabelPositionAct=defaultSelectedValue(driver,X_Label_Position_Input);
			 if(Default_X_LabelPosition_BarWithLine.equals(default_XLabelPositionAct)) {
				 pass(driver,"By default "+Default_X_LabelPosition_BarWithLine+" is displayed in the X Label Position input");
			 }else {
				 fail(driver,"By default "+Default_X_LabelPosition_BarWithLine+" is not displayed in the X Label Position input");
			 }
			 
			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_028");
			 //Verifying the X Label Position input is Accessible
			 ...
			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_029");
			 //Changing the X Lable postion value and verify it is Applied in the chart
			 selectByText(driver, X_Label_Position_Input, Change_X_Lable_Position_ChartStyle);
			 wait(driver,"1");
			 String afterselect=defaultSelectedValue(driver, X_Label_Position_Input);
			 if(afterselect.equals(Change_X_Lable_Position_ChartStyle)) {
			 ...
			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_030");
			 //Getting the default selected Grid Distance Value from the input Element and Verify with Expected Default Value 
			 String defaultGrid_distance_Act=defaultSelectedValue(driver,Grid_Distance_Input);
			 if(Default_GridDistance_BarWithLine.equals(defaultGrid_distance_Act)) {
				 pass(driver,"By default "+Default_GridDistance_BarWithLine+" is displayed in the Grid Distance Input");
			 }else {
				 fail(driver,"By default "+Default_GridDistance_BarWithLine+" is not displayed in the Grid Distance Input");
			 }
			 
			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_031");
			 //Validating the Grid Distance input is Accessible
			 ...
			 setTestCaseID("TC_BARWITHLINE_ CHART_STYLE_032");
			 //Changing the Grid Distance Value based on the input from the Excel sheet and Validate the changed value is applied
			 selectByText(driver, Grid_Distance_Input, Change_Grid_Distance_ChartStyle);
			 wait(driver,"1");
			 String afterselectGrid=defaultSelectedValue(driver, Grid_Distance_Input);
			 if(afterselectGrid.equals(Change_Grid_Distance_ChartStyle)) {
