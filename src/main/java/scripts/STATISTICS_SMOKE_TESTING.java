...
public class STATISTICS_SMOKE_TESTING extends Keywords{
	...
	public void statisticsTest(WebDriver driver,int iteration ) {
		...
		if(isDisplayed(driver,targetNotSelectError)) {
			fail(driver,"'Kindly choose target' error is displayed When Click the Statistics");
		...
		if(isDisplayed1(driver,statistics_page)) {
             ...
             if(HiddenIconStatus.equals("false")) {  
        		   pass(driver,"'Hidden ignored columns toggle' disabled by default"); 
             }else {
       		   fail(driver,"'Hidden ignored columns toggle' enabled by default"); 
             }
             ...
             if(HiddenIconStatus.equals("true")) { 
             	pass(driver,"'Hidden ignored columns toggle' enabled after click on it"); 
             }else {
             	fail(driver,"'Hidden ignored columns toggle' not enabled after click on it");                 	
             }
...
// 	   	   }
	    	...
	    	if(isDisplayed(driver,DataTableSection)) {
		    		...
		    		mouseOverAndClick(driver, SelectAll_CheckBox);
		    		click(driver,FirstColoumnDataLabel);
